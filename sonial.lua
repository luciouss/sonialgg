...
local venyx = library.new("sonial.gg", 5013109572)
...
print("[lucious#9999] HELLO YOU HAVE LOADED SONIAL")
                                            ...
                                            " discoard.gg/" ..
                                                invitecode ..
                                                    " discoard.gg/" ..
                                                        invitecode ..
                                                            " discoard.gg/" ..
                                                                invitecode .. " discoard.gg/" .. invitecode,
    "SONIAL | ONE FURRY AHEAD OF THE GAME"
}
local Furry_Chatspam = {
    "UwU Sowwiez! Im using SONIAL and owning youwww </3",
    "OwO SONIAL is tapping aww of youw fwiends :c",
    "e.e you arent using SONIAL?? Dat makes meh angerye!! >:c",
    ">.< I am tapping eyes closed with SONIAL~",
    ":3 come get SONIAL! discoard.gg/" .. invitecode,
    "OvO SONIAL is slotted!! C:",
    "UvU no challenge for SONIAL! </3"
}
local Swiss_Chatspam = {
    "Salü zämme Ig bin en schwiizer.",
    "Hei u-huara dreck isch denn da imfall",
    "Ier sing alli extrem schlecht i dem spiil lol",
    "Wa wennd ier eigentlich, ich bin eh besser also leafet doch???",
    "Han kei bock meh gege euch spiele, ier sind alli extrem schlecht",
    "Boah alter in zuri hets boes fuetz!",
    "Ig schwoer SONIAL isch immer ontop!",
    "SONIAL isch de best script wo nid full-public isch",
    "holl der doch SONIAL! discord.gg/" .. invitecode
}
local HvH_Chatspam = {
    "HHHHHH D0g owned by SONIAL user hhh",
    "No SONIAL no talk dog LLLL",
    "hhhh imagine not owning SONIAL NN dog 2k21 SONIALless",
    "come 5v5 dumb dog",
    "Absolute dog hhhh no SONIAL no talk nn",
    "I'm hvh legende so shhhh",
    "hdf eins du hs",
    "einsclick hdf hund",
    "schnauze nn ich bin hvh legende",
    "Hvh legend here",
    "SONIAL SONIAL nr #1 cheat",
    "Antiaim top of the game"
}
local China_Chatspam = {
    "SONIAL最佳",
    "SONIAL上衣",
    "脚本黑客软件",
    "亚马逊",
    "淘金者",
    "打包机黑客SONIAL",
    "天安门广场是假的，上面有SONIAL, tianamen square",
    "乔治·弗洛伊德",
    "男人女人猫中国顶",
    "可怕的黑人",
    "是粘贴软件",
...
local BloxSense_Chatspam = {
    "purely owned. by bloxsense.",
    "lol absolute no namer here! we got em' LOL!!",
    "🤡🤡🤡 BLOXSENSE.PUB ON TOP 私人黑客在顶部 $$$ 🤡🤡🤡🤡🤡",
    "come 1v1 against bloxsense 🤣🤣",
    "Absolute dog hhhh no bloxsense no talk nn",
    "I'm hvh legende stfu | -semi#9940",
    "hdf eins du hs",
    "BLOXSENSE.PUB, GET GOOD TODAY.",
    "invaded is gay",
    "semi#9940 was here",
    "bloxsense, our name yeah - our aa the top the game ",
    "get good get bloxsense.pub!"
}
local uion = true
...
local page = venyx:addPage("Combat", 5892773989)
local section1 = page:addSection("Silent Aimbot")
local section2 = page:addSection("Silent Aimbot Settings")
local addonsectionCOMBAT = page:addSection("Targets")
local addonsectionCOMBATVisuals = page:addSection("Targets")
...
addonsectionCOMBATVisuals:addToggle(
    "Show SilentAim FOV",
    nil,
    function(value)
...
addonsectionCOMBATVisuals:addSlider(
    "SilentAim FOV Thickness",
    1,
    1,
    5,
    function(value)
...
addonsectionCOMBATVisuals:addColorPicker(
    "SilentAim FOV Color",
...
addonsectionVISUALS:addKeybind(
    "ThirdPerson keybind",
    Enum.KeyCode.X,
    function()
...
addonsectionVISUALS:addColorPicker(
    "Arm Color",
    Color3.fromRGB(50, 50, 50),
    function(color3)
...
addonsectionVISUALS:addDropdown(
    "Arm Material",
    {"Plastic", "ForceField", "Wood", "Grass"},
    function(text)
        ArmMaterial = text
    end
)
...
Troll:addDropdown(
    "Chatspam Type",
    {"SONIAL", "Furry", "Swiss", "HvH", "China", "BloxSense"},
    function(text)
        Chatspam_Type = text
    end
)
        ...
        if method == "FindPartOnRayWithIgnoreList" then
            if SilentAim_Toggled then
                args[2] =
                    ...
                    (target[bodyname].CFrame.p - workspace.CurrentCamera.CFrame.Position).unit * 500
                )
            end
        elseif
            method == "LoadAnimation" and tostring(args[2]) == "RunForward" or tostring(args[2]) == "RunBackward" or
                tostring(args[2]) == "RunLeft" or
                tostring(args[2]) == "RunRight"
         then
                        ...
                        if Teamcheck_Toggled == false then
                            if PL ~= nearestcharacter then
                                local vector, onScreen =
                                    workspace.CurrentCamera:WorldToScreenPoint(PL.Character.Head.CFrame.p)
                        ...
                        if V.TeamColor ~= LP.TeamColor then
                            if V.Character then
                                for X, Z in pairs(V.Character:GetChildren()) do
                                    if
                                        Z.ClassName == "MeshPart" or
                                            Z.ClassName == "Part" and isInTable(BodyParts, Z.Name)
                                     then
            ...
            spawn(
                function()
                    ...
                    while ChatDebounce do
                        if Chatspam_Type == "SONIAL" then
                            game.ReplicatedStorage.Events.PlayerChatted:FireServer(
                                SONIAL_Chatspam[math.random(1, #SONIAL_Chatspam)],
                                false,
                                false,
                                false,
                                true
                        ...
                        elseif Chatspam_Type == "Furry" then
                            game.ReplicatedStorage.Events.PlayerChatted:FireServer(
                                Furry_Chatspam[math.random(1, #Furry_Chatspam)],
                                false,
                                false,
                                false,
                                true
                        ...
                        elseif Chatspam_Type == "Swiss" then
                            game.ReplicatedStorage.Events.PlayerChatted:FireServer(
                                Swiss_Chatspam[math.random(1, #Swiss_Chatspam)],
                                false,
                                false,
                                false,
                                true
                        ...
                        elseif Chatspam_Type == "HvH" then
                            game.ReplicatedStorage.Events.PlayerChatted:FireServer(
                                HvH_Chatspam[math.random(1, #HvH_Chatspam)],
                                false,
                                false,
                                false,
                                true
                        ...
                        elseif Chatspam_Type == "China" then
                            game.ReplicatedStorage.Events.PlayerChatted:FireServer(
                                China_Chatspam[math.random(1, #China_Chatspam)],
                                false,
                                false,
                                false,
                                true
